...
const Dashboard = () => {
  ...
  const { streak, streakSaved } = useUserStats();
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex justify-center">
        <div className="w-full max-w-md bg-gradient-to-br from-amber-400 to-orange-500 rounded-[2.5rem] p-8 shadow-lg shadow-orange-500/20 text-white flex flex-col justify-between relative overflow-hidden group">
          <div className="absolute -bottom-4 -right-4 opacity-20 rotate-12 group-hover:scale-110 transition-transform">
            <Clock size={150} />
          </div>
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em] opacity-80">{t('dailyStreak')}</span>
            </div>
            <div className="flex items-baseline gap-2">
              <h2 className="text-6xl font-black tabular-nums tracking-tighter">{streak}</h2>
              <span className="text-sm font-black uppercase tracking-widest opacity-80">{t('days')}</span>
            </div>
          </div>
          <div className="relative z-10 mt-6">
            {streak === 0 ? (
              <p className="text-[10px] font-black uppercase tracking-widest bg-white/20 py-2 px-4 rounded-full inline-block backdrop-blur-md">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap">{t('keepItUp')}!</span>
              </div>
            )}
          </div>
        </div>
